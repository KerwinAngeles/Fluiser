...
import type { Habit, CategoryId, HabitIcon, Tone, CategoryDef } from '@/types'
...
<template>
  <BaseModal :open="!!habit" width="min(1180px, 96vw)" @close="emit('close')">
    <div v-if="cachedId" class="heb">
      <!-- Header -->
      <div class="heb-head" :style="{ backgroundImage: `radial-gradient(120% 140% at 12% 0%, var(--${tone}-soft), transparent 65%)` }">
        <div class="row" style="align-items: center; gap: 18px; min-width: 0;">
          ...
          <div style="min-width: 0;">
            <div class="card-title" style="margin-bottom: 8px;">
              {{ cachedIsNew ? t('Nuevo hábito', 'New habit') : t('Editar hábito', 'Edit habit') }} · {{ categoryLabel }}
            </div>
            <input
              class="heb-name"
              v-model="name"
        ...
        <button class="btn btn-ghost btn-sm" style="flex-shrink: 0;" @click="emit('close')">
          <XIcon :size="14" />
        </button>
      </div>
...
.heb-name {
  ...
  min-width: 280px; max-width: 100%; width: 100%;
...
.heb-foot {
  ...
  padding: 22px 36px; border-top: 1px solid var(--border-subtle);
}

@media (max-width: 860px) {
  .heb-body { grid-template-columns: 1fr; }
  .heb-name { font-size: 22px; min-width: 0; }
  .heb-head, .heb-body, .heb-foot { padding-left: 22px; padding-right: 22px; }
}
</style>
